...
function writeSitemap(
  ...
  // Keep the served copy (public/) identical to the built one, so dev preview,
  // robots.txt and llms.txt always point at the same, current URL set.
  fs.writeFileSync(path.resolve("public/sitemap.xml"), xml);
  console.log(`[seo-plugin] Wrote sitemap.xml (${urls.length} URLs)`);

  // robots.txt is generated by vite-plugin-llm-assets during buildEnd, i.e.
  // after public/ was copied into dist/. Re-copy so dist ships the fresh one.
  const robotsSrc = path.resolve("public/robots.txt");
  if (fs.existsSync(robotsSrc)) {
    let robots = fs.readFileSync(robotsSrc, "utf-8");
    robots = robots.replace(/^# sitemap\.xml: \d+ URL$/m, `# sitemap.xml: ${urls.length} URL`);
    fs.writeFileSync(robotsSrc, robots);
    fs.writeFileSync(path.join(distDir, "robots.txt"), robots);
    console.log("[seo-plugin] Synced robots.txt into dist/");
  }
  for (const f of ["llms.txt", "llms-full.txt", "llms-geo.txt"]) {
